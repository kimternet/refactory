...
def integer_integer_calculation():
    ...
    return f"\\({correct_expression}\\)", f'\\({correct_result_str}\\)', f"\\({correct_calc}\\)"
...
def decimal_decimal_calculation():
    ...
    correct_result_str = (
        ...
        else f"{round(correct_result, 1):+}" if correct_result < 0
        else f"{round(correct_result, 1)}"
    )
    correct_expression = f"({num1:+})-({num2:+})"
    correct_calculation = f"({num1:+}) - ({num2:+}) = ({num1:+}) + ({-num2:+}) = {correct_result_str}"

    return f"\\({correct_expression}\\)", f"\\({correct_result_str}\\)", f"\\({correct_calculation}\\)"
...
def integer_fraction_calculation_with_multiplication():
    ...
    if random.choice([True, False]):
        ...
        num1_as_fraction_str = f"{sign}\\frac{{{abs(neg_num1 * fraction.denominator)}}}{{{abs(fraction.denominator)}}}"
    ...
    if result != simplified_result:
        ...
        simplified_result_str = f"{sign}\\frac{{{abs(simplified_result.numerator)}}}{{{simplified_result.denominator}}}" if simplified_result.numerator > 0 else f"-\\frac{{{simplified_result.numerator}}}{{{simplified_result.denominator}}}"
        answer = simplified_result_str
    else:
        answer = result_str
