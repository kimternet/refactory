...
from .utils import (
    ...
    format_fraction_with_sign,
    generate_random_sign_fraction,
    format_fraction_with_sign_answer,
    format_fraction_with_sign_answer2
)
...
def decimal_decimal_calculation():
   ...
   expression = f"({num1:+}) - ({num2:+})"
   calculation = f"({num1:+}) - ({num2:+}) = ({num1:+}) + ({-num2:+}) = {result_str}"
   return f"\\({expression}\\)", f"\\({result_str}\\)" if result >= 0 else f"\\({result_str}\\)", f"\\({calculation}\\)"
